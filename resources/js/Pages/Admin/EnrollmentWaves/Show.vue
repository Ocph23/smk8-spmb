    ...
    <AdminLayout>
        <div class="mx-auto max-w-4xl space-y-6">
            ...
            <div class="rounded-lg bg-white p-6 shadow-sm">
                <h2 class="mb-4 text-base font-medium text-gray-700">Detail Gelombang</h2>
                ...
                <div v-else class="grid grid-cols-2 gap-4 text-sm text-gray-700 sm:grid-cols-4">
                    <div><span class="block text-xs text-gray-500">Tanggal Buka</span>{{ wave.open_date ?? '-' }}</div>
                    <div><span class="block text-xs text-gray-500">Tanggal Tutup</span>{{ wave.close_date ?? '-' }}</div>
                    <div class="col-span-2"><span class="block text-xs text-gray-500">Deskripsi</span>{{ wave.description ?? '-' }}</div>
                </div>
            </div>

            <!-- Quota Table -->
            <div class="rounded-lg bg-white p-6 shadow-sm">
                <h2 class="mb-4 text-base font-medium text-gray-700">Kuota per Jurusan</h2>
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b text-left text-xs text-gray-500">
                            <th class="pb-2 font-medium">Jurusan</th>
                            <th class="pb-2 font-medium text-right">Kuota</th>
                            <th class="pb-2 font-medium text-right">Diterima</th>
                            <th class="pb-2 font-medium text-right">Sisa</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="stat in quotaStats" :key="stat.major_id" class="border-b last:border-0">
                            <td class="py-2 text-gray-800">{{ stat.major_name }}</td>
                            <td class="py-2 text-right">
                                <input v-if="wave.status === 'draft'" v-model.number="quotaValues[stat.major_id]"
                                    type="number" min="0"
                                    class="w-20 rounded border border-gray-300 px-2 py-1 text-right text-sm" />
                                <span v-else>{{ stat.quota }}</span>
                            </td>
                            <td class="py-2 text-right text-gray-600">{{ stat.accepted }}</td>
                            <td class="py-2 text-right" :class="stat.remaining === 0 ? 'text-red-600 font-medium' : 'text-green-700'">
                                {{ stat.remaining }}
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div v-if="wave.status === 'draft'" class="mt-4">
